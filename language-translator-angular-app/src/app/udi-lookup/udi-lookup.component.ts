      ...
      this.pdfLookupService
        ...
        .subscribe({
          next: (data) => {
            ...
            console.log("UdiLookupComponent: pdfDoc: ", "name : ", this.pdfDoc.name, " id: ", this.pdfDoc.id, " length :", this.pdfDoc.base64Data.length);    
            this.dataSharingService.setSrcDoc(this.pdfDoc);
            // show message
            this.showMessageDocumentFound(true);
            console.log("UdiLookupComponent: pdfDoc stored in Data-Sharing-Svc");
          }
          , error: (error) => {
            // Handle error
